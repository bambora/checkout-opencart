...
class ControllerExtensionPaymentBamboraOnlineCheckout extends Controller
{
    ...
    public function index()
    {
        ...
        $data = array();
        $data['text_title'] = $this->config->get(
            $this->getConfigBaseName() . '_payment_method_title'
        );
        $data['text_loading'] = $this->language->get('text_loading');
        $data['text_payment'] = $this->language->get('text_payment');
        $data['button_confirm'] = $this->language->get('button_confirm');

        $orderInfo = $this->model_checkout_order->getOrder(
            $this->session->data['order_id']
        );
        $data[$this->module_name . '_allowed_payment_type_ids'] = $this->getAllowedPaymentTypeIds(
            $orderInfo['currency_code'],
            $orderInfo['total'],
            $orderInfo['order_id']
        );
        $data[$this->module_name . '_window_state'] = $this->config->get(
            $this->getConfigBaseName() . '_window_state'
        );
    ...
    protected function getAllowedPaymentTypeIds($currency, $amount, $orderId)
    {
        ...
        $amountInMinorunits = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
            $amount,
            $minorunits
        );
        $paymentTypeResponse = $this->model_extension_payment_bambora_online_checkout->getPaymentTypeIds(
            $currency,
            $amountInMinorunits
        );
    ...
    public function confirm()
    {
        ...
        $checkoutSessionResponse = $this->model_extension_payment_bambora_online_checkout->setCheckoutSession(
            $checkoutSessionRequest
        );

        $json = array();
        if (!$checkoutSessionResponse || !$checkoutSessionResponse->meta->result) {
            $json['error'] = $this->language->get(
                    'error_payment_window'
                ) . ' ' . $checkoutSessionResponse->meta->message->enduser;
            $this->model_extension_payment_bambora_online_checkout->bamboraLog(
                $this->language->get(
                    'error_payment_window'
                ) . ' ' . $checkoutSessionResponse->meta->message->merchant
            );
        } else {
    ...
    protected function createCheckoutSessionRequest()
    {
        ...
        $orderTotals = array();
        if ($this->is_oc_3()) {
            $orderTotals = $this->model_checkout_order->getOrderTotals(
                $this->session->data['order_id']
            );
        } else {
            $orderTotalTemp = $this->db->query(
                "SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$this->session->data['order_id'] . "'"
            );
        ...
        $params['instantcaptureamount'] = $this->config->get(
            $this->getConfigBaseName() . '_instant_capture'
        ) === "1" ? $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
            $orderTotalAmount,
            $minorunits
        ) : 0;
        $params['paymentwindowid'] = $this->config->get(
            $this->getConfigBaseName() . '_window_id'
        );

        $params['customer'] = array();
        $params['customer']['phonenumbercountrycode'] = html_entity_decode(
            $orderInfo['payment_iso_code_2'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['customer']['phonenumber'] = html_entity_decode(
            $orderInfo['telephone'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['customer']['email'] = html_entity_decode(
            $orderInfo['email'],
            ENT_QUOTES,
            'UTF-8'
        );
        ...
        $params['order']['amount'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
            $orderTotalAmount,
            $minorunits
        );
        $params['order']['vatamount'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
            $orderTaxAmount,
            $minorunits
        );

        $params['order']['currency'] = $orderInfo['currency_code'];
        $params['order']['shippingaddress'] = $this->createCustomerAddress(
            $orderInfo,
            'shipping'
        );
        $params['order']['billingaddress'] = $this->createCustomerAddress(
            $orderInfo,
            'payment'
        );
        $params['order']['lines'] = $this->createOrderLines(
            $orderTotals,
            $minorunits
        );

        $params['url'] = array();
        $params['url']['immediateredirecttoaccept'] = $this->config->get(
            $this->getConfigBaseName() . '_immediate_redirect_to_accept'
        );
        $params['url']['accept'] = $this->url->link(
            'extension/payment/bambora_online_checkout/accept',
            '',
            true
        );
        ...
        $params['url']['callbacks'][] = array(
            'url' => $this->url->link(
                'extension/payment/' . $this->module_name . '/callback',
                '',
                true
            )
        );
        ...
        $allow_low_value = $this->config->get(
            $this->getConfigBaseName() . '_allow_low_value_exemptions'
        );
        if ($allow_low_value) {
            if ($params['order']['amount'] < $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $this->config->get(
                        $this->getConfigBaseName() . '_limit_for_low_value_exemption'
                    ),
                    $minorunits
                )) {
    ...
    protected function createCustomerAddress($orderInfo, $type)
    {
        $params = array();
        $params['att'] = "";
        $params['firstname'] = html_entity_decode(
            $orderInfo[$type . '_firstname'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['lastname'] = html_entity_decode(
            $orderInfo[$type . '_lastname'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['street'] = html_entity_decode(
            $orderInfo[$type . '_address_1'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['zip'] = html_entity_decode(
            $orderInfo[$type . '_postcode'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['city'] = html_entity_decode(
            $orderInfo[$type . '_city'],
            ENT_QUOTES,
            'UTF-8'
        );
        $params['country'] = html_entity_decode(
            $orderInfo[$type . '_iso_code_3'],
            ENT_QUOTES,
            'UTF-8'
        );
    ...
    protected function createOrderLines($orderTotals, $minorunits)
    {
        $orderProducts = array();
        if ($this->is_oc_3()) {
            $orderProducts = $this->model_checkout_order->getOrderProducts(
                $this->session->data['order_id']
            );
        } else {
        ...
        foreach ($orderProducts as $product) {
            ...
            $lineTotalPriceInclVat = $this->formatForCurrency(
                $lineTotalPriceInclVat
            );

            $line['totalprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $lineTotalPrice,
                $minorunits
            );
            $line['totalpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $lineTotalPriceInclVat,
                $minorunits
            );
            $line['totalpricevatamount'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $lineTaxAmount,
                $minorunits
            );
            $line['unitprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $lineTotalPrice / $product['quantity'],
                $minorunits
            );
            $line['unitpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $lineTotalPriceInclVat / $product['quantity'],
                $minorunits
            );
            $line['unitpricevatamount'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $lineTaxAmount / $product['quantity'],
                $minorunits
            );
            $line['vat'] = round(
                $lineTaxAmount > 0 ? ($lineTaxAmount / $product['total']) * 100 : 0,
                2,
                $this->model_extension_payment_bambora_online_checkout->getRoundingMode(
                )
            );
            $params[] = $line;
            $lineNumber++;
        ...
        if (isset($shipping) && isset($this->session->data['shipping_method'])) {
            ...
            if (!empty($shippingMethod['cost']) && $shippingMethod['cost'] > 0) {
                ...
                $shippingTaxArray = $this->tax->getRates(
                    $shippingMethod['cost'],
                ...
                $shipping['totalprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $shippingTotalPrice,
                    $minorunits
                );
                $shipping['totalpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $shippingWithTax,
                    $minorunits
                );
                $shipping['totalpricevatamount'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $shippingTaxAmount,
                    $minorunits
                );
                $shipping['unitprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $shippingTotalPrice,
                    $minorunits
                );
                $shipping['unitpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $shippingWithTax,
                    $minorunits
                );
                $shipping['unitpricevatamount'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                    $shippingTaxAmount,
                    $minorunits
                );
                $shipping['vat'] = round(
                    $shippingTaxAmount > 0 ? ($shippingTaxAmount / $shippingMethod['cost']) * 100 : 0,
                    2,
                    $this->model_extension_payment_bambora_online_checkout->getRoundingMode(
                    )
                );
        ...
        if (!empty($orderTotalDiscount) && (float)$orderTotalDiscount['value'] < 0) {
            ...
            $couponTotalPrice = $orderTotalDiscount['value'];
            //Format for currency
            $couponTotalPrice = $this->formatForCurrency($couponTotalPrice);

            $coupon['totalprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $couponTotalPrice,
                $minorunits
            );
            $coupon['totalpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $couponTotalPrice,
                $minorunits
            );
            $coupon['totalpricevatamount'] = 0;
            $coupon['unitprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $couponTotalPrice,
                $minorunits
            );
            $coupon['unitpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $couponTotalPrice,
                $minorunits
            );
        ...
        if (!empty($orderTotalVoucher) && (float)$orderTotalVoucher['value'] < 0) {
            $voucher = array();
            $voucher['id'] = $this->language->get(
                'text_voucher_id'
            );//$orderTotalVoucher['code'];
            ...
            $voucher['totalprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $voucherTotalPrice,
                $minorunits
            );
            $voucher['totalpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $voucherTotalPrice,
                $minorunits
            );
            $voucher['totalpricevatamount'] = 0;
            $voucher['unitprice'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $voucherTotalPrice,
                $minorunits
            );
            $voucher['unitpriceinclvat'] = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits(
                $voucherTotalPrice,
                $minorunits
            );
    ...
    public function accept()
    {
        ...
        if ($this->validateRequest($getParameteres, $message)) {
            $orderInfo = $this->model_checkout_order->getOrder(
                $getParameteres['orderid']
            );
            if ($orderInfo['order_status_id'] !== $this->config->get(
                    $this->getConfigBaseName() . '_order_status_completed'
                )) {
    ...
    public function callback()
    {
        ...
        try {
            ...
            if (!$this->validateCallback($getParameteres, $message, $transaction)) {
                $orderId = array_key_exists(
                    'orderid',
                    $getParameteres
                ) ? $getParameteres['orderid'] : -1;
                ...
                header(
                    'X-EPay-System: ' . $this->model_extension_payment_bambora_online_checkout->getModuleHeaderInformation(
                    ),
                    true,
                    500
                );
            ...
            if (($dbTransaction && !empty($dbTransaction['transaction_id'])) || $orderInfo['order_status_id'] === $this->config->get(
                    $this->getConfigBaseName() . '_order_status_completed'
                )) {
                header(
                    'X-EPay-System: ' . $this->model_extension_payment_bambora_online_checkout->getModuleHeaderInformation(
            ...
            $amount = $this->model_extension_payment_bambora_online_checkout->convertPriceFromMinorunits(
            ...
            if ($this->is_oc_3() && ($this->config->get(
                        $this->getConfigBaseName() . '_surcharge'
                    ) == 1 && $transaction->total->feeamount > 0)) {
                $this->addSurchargeToOrderTotals(
                    $transaction,
            ...
            $this->model_extension_payment_bambora_online_checkout->addDbTransaction(
                $orderId,
                $transaction->id,
                $transaction->total->authorized,
            ...
            $comment .= '<tr><td>' . '<b>' . $this->language->get(
                    'payment_process'
                ) . '</b></td><td>' . $transaction->currency->code . ' ' . $amount . '</td></tr>';
            $comment .= '<tr><td>' . '<b>' . $this->language->get(
                    'payment_with_transactionid'
                ) . '</b></td><td>' . $transaction->id . '</td></tr>';
            $comment .= '<tr><td>' . '<b>' . $this->language->get(
                    'payment_card'
                ) . '</b></td><td>' . $paymentInfo . '</td></tr>';
            $comment .= '</tbody></table>';
    ...
    protected function addSurchargeToOrderTotals($transaction, $currentStatusId)
    {
        ...
        $transactionFee = $this->model_extension_payment_bambora_online_checkout->convertPriceFromMinorunits(
            $baseTransactionFee,
        ...
        if ($this->is_oc_3()) {
            ...
            $orderTotalTemp = $this->db->query(
                "SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$transaction->orderid . "'"
            );
            $orderTotals = isset($orderTotalTemp) ? $orderTotalTemp->rows : array();
        }
        if (!empty($orderTotals)) {
            ...
            if (count($orderTotals) > 1) {
                $this->db->query(
                    "DELETE FROM " . DB_PREFIX . "order_total WHERE order_id = '" . (int)$transaction->orderid . "'"
                );
                ...
                foreach ($orderTotals as $total) {
                    ...
                    if ($total['code'] === 'total') {
                        ...
                        $this->db->query(
                            "UPDATE " . DB_PREFIX . "order SET total = '" . $total['value'] . "' WHERE order_id = '" . (int)$transaction->orderid . "'"
                        );
                    }
                    $this->db->query(
                        "INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$transaction->orderid . "', code = '" . $this->db->escape(
                            $total['code']
                        ) . "', title = '" . $this->db->escape(
                            $total['title']
                        ) . "', `value` = '" . (float)$total['value'] . "', sort_order = '" . (int)$total['sort_order'] . "'"
                    );
                }
                $transactionFeeFormatted = $this->currency->format(
                    $transactionFee,
                    $transaction->currency->code,
                    false,
                    true
