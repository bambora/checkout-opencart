...
class ControllerExtensionPaymentBamboraOnlineCheckout extends Controller {
	...
	protected function createOrderLines( $orderTotals, $minorunits ) {
		...
		if ( ! empty( $orderTotalDiscount ) && (float) $orderTotalDiscount['value'] < 0 ) {
			...
			$coupon['totalprice']          = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits( $couponTotalPrice, $minorunits );
			$coupon['totalpriceinclvat']   = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits( $couponTotalPrice, $minorunits );
			$coupon['totalpricevatamount'] = 0;
			$coupon['unitprice']           = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits( $couponTotalPrice, $minorunits );
			$coupon['unitpriceinclvat']    = $this->model_extension_payment_bambora_online_checkout->convertPriceToMinorunits( $couponTotalPrice, $minorunits );
