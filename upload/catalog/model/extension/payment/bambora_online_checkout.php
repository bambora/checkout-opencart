...
class ModelExtensionPaymentBamboraOnlineCheckout extends Model
{
        ...
        if ($status) {
            $method_data = array(
                ...
                'sort_order' => $this->config->get(
                    $this->getConfigBaseName() . '_sort_order'
                )
            );
        }
    ...
    public function addDbTransaction($orderId, $transactionId, $amount, $currency)
    {
        try {
            $this->db->query(
                "INSERT INTO " . DB_PREFIX . "bambora_online_checkout_transaction SET
         ...
         "
            ...
            throw new Exception(
                "Could not add the transaction to the database. Possible dublicate entry txnid: {$transactionId} and orderId: {$orderId}"
            );
        }
    }
    ...
    public function getDbTransaction($orderId)
    {
        $result = $this->db->query(
            "SELECT * FROM " . DB_PREFIX . "bambora_online_checkout_transaction WHERE order_id = " . (int)$this->db->escape(
                $orderId
            )
        );
    ...
    /**
     * Get the Bambora API key based on access token, merchant number and secret
     * token
     *
     * @return string
     */
    ...
    {
        $accesstoken = $this->config->get(
            $this->getConfigBaseName() . '_access_token'
        );
        $merchantNumber = $this->config->get(
            $this->getConfigBaseName() . '_merchant'
        );
        $secretToken = $this->config->get(
            $this->getConfigBaseName() . '_secret_token'
        );
    ...
    public function convertPriceToMinorunits($amount, $minorunits)
    {
        ...
        $roundingMode = $this->config->get(
            $this->getConfigBaseName() . '_rounding_mode'
        );
        switch ($roundingMode) {
            case 'up':
     ...
     * @return string
     */
    public function convertPriceFromMinorunits(
        $amount,
        $minorunits,
        $decimalPoint = '.',
        $thousandSeparator = ''
