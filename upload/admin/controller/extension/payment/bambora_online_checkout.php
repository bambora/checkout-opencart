    ...
    class ControllerExtensionPaymentBamboraOnlineCheckout extends Controller
    {
        ...
        protected function validate()
        {
            if ( ! $this->user->hasPermission(
                ...
                $this->errors['permission'] = $this->language->get(
                    'error_permission'
                );
            } else {
        ...
        protected function initSettingsContent()
        {
            ...
            $this->data['column_left']    = $this->load->controller(
                'common/column_left'
            );
        ...
        public function getPaymentTransaction()
        {
            $this->load->language('extension/payment/' . $this->module_name);
            $data         = array();
            $moduleStatus = $this->config->get(
                $this->getConfigBaseName() . '_status'
            );
            if ($moduleStatus && isset($this->request->get['order_id'])) {
                ...
                if ($dbTransaction && ! empty($dbTransaction['transaction_id'])) {
                    ...
                    if ($transactionResponse && $transactionResponse->meta->result) {
                        ...
                        $data['text_btn_void']                         = $this->language->get(
                            'text_btn_void'
                        );
                        ...
                        $data['text_no']                               = $this->language->get(
                            'text_no'
                        );
                        $data['text_yes']                              = $this->language->get(
                            'text_yes'
                        );
                        $data['text_tooltip']                          = $this->language->get(
                            'text_tooltip'
                        );
                        ...
                        $data['date_format']                           = $this->language->get(
                            'date_format'
                        );
                        ...
                        $decimalPoint      = $this->language->get(
                        ...
                        if (isset($transaction->information->wallets[0]->name)) {
                            ...
                            if (isset($wallet_img)) {
                                $card_image .= '&nbsp;<img style="max-height:51px;clear: both;" src="https://d3r1pwhfz7unl9.cloudfront.net/paymentlogos/' . $wallet_img . '" alt="' . $wallet_name . '" title="' . $wallet_name . '" />';
                            }
                        }
                    ...
                    $data['getPaymentTransaction_success']    = false;
                    $errorMessage                             = $this->language->get(
                        'error_get_transaction_db'
                    );
                    $data['text_getPaymentTransaction_error'] = $errorMessage;
                    $this->model_extension_payment_bambora_online_checkout->bamboraLog(
                        $errorMessage
